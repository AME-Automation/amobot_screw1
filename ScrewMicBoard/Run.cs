...
public class RunState
{
	...
	private int ScrewNum = 5;
	private int AlignArrayMaxCount = 4;
	...
	private double[] ActualScrewPos5;
	private double[] ScrewThrowPos;
	private double[] BarcodePos;
	//private double[][] ScrewPositions = new double[5][]; //supposed to use screwnums
	//private double[][] ActualScrewPos;
	
	private bool Detect = false;//视觉是否正常执行Detect
	private bool Capture = false;//视觉是否正常执行Capture
	
	/* Need to change above "Capture" to below
	...
	*/ 
	...
	private double Screw_1_dx = 0;//Capture的四个点位坐标值
	private double Screw_1_dy = 0;
	private double Screw_2_dx = 0;
	private double Screw_2_dy = 0;
	private double Screw_3_dx = 0;
	private double Screw_3_dy = 0;
	private double Screw_4_dx = 0;
	private double Screw_4_dy = 0;
	private double Screw_5_dx = 0;
	private double Screw_5_dy = 0;
	private double [] screw_dx;
	private double [] screw_dy;
	...
	public int Exec(ITask task)
	{
		...
		try
		{
			while(task.TaskRunStat!=TaskRunState.Stop)
			{
				...
				var val=DataInfoManager.Instance.DataInfoGetVal("TrayReady"); //tray ready bool is checked everytime.
				if(val!=null)
				{
					TrayReady=(bool)val;
					//TrayReady = true; //set for debug
				}
				...
				switch(CurStep)
				{
					...
					case 10://ScrewDown Retract and AxisGroup jump to PickScrew 
						...
						if(CheckAutoRun())
						{
							...
							{
								//((IEleCyldLine)_eleCylScrewDown).CyldRetract();
								if(true)//((IEleCyldLine)_eleCylScrewDown).ReadRestractDI()==1)
								{
					...
					case 50://CCDLight2 on，take picture to check screw ready
						...
						if(CheckAutoRun())
						{
							...
							if(!CheckDryRun()) //auto run is here, change to avoid error detection
							{
					...
					case 80://CCDLight1 on，take picture to check product ready
						...
						if(isScrewPicTaken == false)
						{
							if(TrayReady&&CheckAutoRun())
							{
								...
								else //in dryrun, skip all image processing
								{
									((IEleDO)_eleDoTrigLight1).Write(true);
									Thread.Sleep(50);
									...
									ActualScrewPos5=new double[]{ScrewPos5[0], ScrewPos5[1], ScrewPos5[2] - 5};
									...
									for (int i=0;i<ScrewNum;i++){
										for(int j=0;j<axisCount;j++){
											if (j==2){
												ScrewPositions[i][j] = ScrewPositions[i][j]-5; //for safety, raise nozzle 5 points
											}
											ActualScrewPos[i][j] = ScrewPositions[i][j]; 
										}
									}*/
									SetStep(90,"AxisGroup move to work position");
								}
					...
					case 81:  //Offset get CapResult filled in OnAlignRecievedHandle
						...
						{
							((IEleDO)_eleDoTrigLight1).Write(false);
							ActualScrewPos1=new double[]{ScrewPos1[0] + Screw_1_dx, ScrewPos1[1] + Screw_1_dy, ScrewPos1[2]};
							ActualScrewPos2=new double[]{ScrewPos2[0] + Screw_2_dx, ScrewPos2[1] + Screw_2_dy, ScrewPos2[2]};
							ActualScrewPos3=new double[]{ScrewPos3[0] + Screw_3_dx, ScrewPos3[1] + Screw_3_dy, ScrewPos3[2]};
							ActualScrewPos4=new double[]{ScrewPos4[0] + Screw_4_dx, ScrewPos4[1] + Screw_4_dy, ScrewPos4[2]};
							//ActualScrewPos5=new double[]{ScrewPos5[0], ScrewPos5[1], ScrewPos5[2]}; //filled in step 83
							UpCamErrCount=0;
							Capture = false;
							CapCalcFinish = false;
							if (ScrewNum <= AlignArrayMaxCount){
								SetStep(90,"AxisGroup move to work position");
							} else {
								SetStep(82); //take 5th screw picture
							}
							//MessageBox.Show("take 2nd pic step 81");
						}
						else 
						{
					...
					case 8000:
						...
						if(CheckAutoRun())
						{
		    				...
		    				{
		    					DialogResult dr=MessageBox.Show(string.Format("AutoRun 8000 Err:Up Camera photograph failed! retry？\n lastStep: {0}", LastStep.ToString()), 
		    					                                "Information", MessageBoxButtons.RetryCancel, MessageBoxIcon.Question);
						...
						#endregion
						break;
					case 82: //take image to find 5th screw
						#region
						//MessageBox.Show("entered step 82");
						if(TrayReady&&CheckAutoRun())
						{
							if(!CheckDryRun())
							{
								((IEleDO)_eleDoTrigLight1).Write(true);
								Thread.Sleep(50);
								if(((IVpAssemble)_vpAssemble).SetCapture(1,0,0,PictureNum++,"007","Station6")==false) //capture using clamp0, station0
								{
									SetStep(8000);
								}
								else
								{
									...
									upPicTakeTime=DateTime.Now;
									SetStep(83);
									//MessageBox.Show("take 2nd picture, step 82");
								}
							} else {
								((IEleDO)_eleDoTrigLight1).Write(true);
									Thread.Sleep(50);
						...
						#endregion
						break;
					case 83: //wait for info and fill in 5th screw
						#region
						if(Capture && CapCalcFinish) //these are set by seperate events, here is reset in step 81
						{
							//MessageBox.Show("entered step 83");
							((IEleDO)_eleDoTrigLight1).Write(false);
							ActualScrewPos5=new double[]{ScrewPos5[0] + Screw_5_dx, ScrewPos5[1] + Screw_5_dy, ScrewPos5[2]}; 
							UpCamErrCount=0;
							SetStep(90,"AxisGroup move to work position");
							//MessageBox.Show("go to step 90");
						}
						else 
						{
							if(isTimeOut(upPicTakeTime,3)==true)
					...
					case 112://Detect the product
						...
						if(CheckAutoRun())
						{
							if(!CheckDryRun())
							{
								...
								if(((IVpAssemble)_vpAssemble).SetCapture(0,0,1,PictureNum++,"007","Station6")==false) //set capture using clamp0 station1
								{
									SetStep(1130);
								}
								else
							...
							else// if(CheckDryRun())
							{
								((IEleDO)_eleDoTrigLight1).Write(true);
								Thread.Sleep(50);
								((IVpAssemble)_vpAssemble).SetCapture(0,0,1,PictureNum++,"007","Station6");
								Thread.Sleep(500);
								((IEleDO)_eleDoTrigLight1).Write(false);
					...
					case 113://等待结果
						#region
						if(Capture) //if image processing is passed
						{
							((IEleDO)_eleDoTrigLight1).Write(false);
							UpCamEndErrCount=0;
					...
					case 1130:
						...
						if(CheckAutoRun())
						{
		    				...
		    				{
		    					MessageBox.Show(string.Format("step 113 \n Capture: {0} \n CapFin {1}", Capture.ToString(), CapCalcFinish.ToString()));
						...
						#endregion
						break;
					case 120://轴系移动至等待位 // use this step to take picture of barcode
						#region
						if(CheckAutoRun())
						{
							
							var ret=((IEleAxisGroup)_eleMotion).Jump(AxisGroup,BarcodePos,MoveVelGroup,-100,100000,false,-1000);
							if(ret==0)
							{
								SetStep(121); //capture
							}
							...
							{
								SetTaskError(true,AlarmLv.Error,510,"AxisGroup jump to BarcodePos failed",task,Exceptions.AxisMove);
								SetStep(-120);
								goto RunStepErr;
							}
					...
					case 121: //capture barcode
						if(CheckAutoRun()){
							((IEleDO)_eleDoTrigLight1).Write(true);
								Thread.Sleep(50);
								((IVpAssemble)_vpAssemble).SetCapture(1,0,1,PictureNum++,"007","Station6");
								Thread.Sleep(500);
								((IEleDO)_eleDoTrigLight1).Write(false);
								SetStep(130);
						}
					...
					case 130://整体完成-Success
						...
						DataInfoManager.Instance.DataInfoSetVal("Screw5TryTime",Screw5TryTime);
	...
	public int Init(ITask task)
	{
		...
		/*for(int i=0;i<ScrewNum;i++){
			ScrewPositions[i] = new double[axisCount];
		}*/
        ...
        ScrewPos1=pos.Data();
        //ScrewPositions[0]=new double[]{ScrewPos1[0], ScrewPos[1], ScrewPos[2]};
        /*for (int i=0;i<axisCount;i++){
        	ScrewPositions[0][i]=ScrewPos1[i];
        }*/
        pos = ElesManager.Instance.ListPos.FirstOrDefault(p => p.Name == "ScrewPos2");
        ScrewPos2=pos.Data();
        //ScrewPositions[1]=pos.Data();
        pos = ElesManager.Instance.ListPos.FirstOrDefault(p => p.Name == "ScrewPos3");
        ScrewPos3=pos.Data();
        //ScrewPositions[2]=pos.Data();
        pos = ElesManager.Instance.ListPos.FirstOrDefault(p => p.Name == "ScrewPos4");
        ScrewPos4=pos.Data();
        //ScrewPositions[3]=pos.Data();
        pos = ElesManager.Instance.ListPos.FirstOrDefault(p => p.Name == "ScrewPos5");
        ScrewPos5=pos.Data();
        //ScrewPositions[4]=pos.Data();
        pos = ElesManager.Instance.ListPos.FirstOrDefault(p => p.Name == "ScrewThrowPos");
        ScrewThrowPos=pos.Data();
		 pos = ElesManager.Instance.ListPos.FirstOrDefault(p => p.Name == "Barcode");
        BarcodePos=pos.Data();
		#endregion
	...
	public void OnProcessFinished_Handle(object sender)
    {
        ...
        //MessageBox.Show(string.Format("curstep: {0}", CurStep.ToString()));
    ...
    public void OnAlignResultRecieved_Handle(object sender) //handles info for both station 0 captures, but will it handle station 1 captures? Why is station 1 not being used?
    {
    	...
    	if(CurStep==81)
    	{
    		...
    		else
    		{
    			...
    			int alignResultArrayCount = ScrewNum;
    			if (alignResultArrayCount > AlignArrayMaxCount){ //we have 5 screws, but the first pic can only accomodate 4 data points
    				alignResultArrayCount = AlignArrayMaxCount; // =4
    			}
    			for(int i=0;i<alignResultArrayCount;i++)
    			{
    				if (vpAlignResultArray[i].Result == VpAlignEnum.VpAlign_Finish) //alignResultArray only has 4 elements max
        			{
            			switch(i)
            			{
            				case 0:
            					Screw_1_dx = vpAlignResultArray[i].Offset.X;
            					Screw_1_dy = vpAlignResultArray[i].Offset.Y;
            					DataInfoManager.Instance.DataInfoSetVal("CapResultX1", Screw_1_dx);
            					DataInfoManager.Instance.DataInfoSetVal("CapResultY1", Screw_1_dy);
            					//CapResultX5 = vpAlignResultArray[i].Offset.R; //linked to R0 in clamp0 station0
            					//MessageBox.Show(string.Format("1st picture taken \n X1: {0} \n Y1: {1}", CapResultX1.ToString(), CapResultY1.ToString()));
            					break;
            				case 1:
            					Screw_2_dx = vpAlignResultArray[i].Offset.X;
            					Screw_2_dy = vpAlignResultArray[i].Offset.Y;
            					//CapResultY5 = vpAlignResultArray[i].Offset.R; //linked to R1 in clamp0 station0
            					DataInfoManager.Instance.DataInfoSetVal("CapResultX2", Screw_2_dx);
            					DataInfoManager.Instance.DataInfoSetVal("CapResultY2", Screw_2_dy);
            					break;
            				case 2:
            					Screw_3_dx = vpAlignResultArray[i].Offset.X;
            					Screw_3_dy = vpAlignResultArray[i].Offset.Y;
            					//CapResultX5 = vpAlignResultArray[i].Offset.R; //linked to R2 in clamp0 station0
            					DataInfoManager.Instance.DataInfoSetVal("CapResultX3", Screw_3_dx);
            					DataInfoManager.Instance.DataInfoSetVal("CapResultY3", Screw_3_dy);
            					//DataInfoManager.Instance.DataInfoSetVal("CapResultX5", CapResultX5);
            					break;
            				case 3:
            					Screw_4_dx = vpAlignResultArray[i].Offset.X;
            					Screw_4_dy = vpAlignResultArray[i].Offset.Y;
            					//CapResultY5 = vpAlignResultArray[i].Offset.R; //linked to R3 in clamp0 station0
            					DataInfoManager.Instance.DataInfoSetVal("CapResultX4", Screw_4_dx);
            					DataInfoManager.Instance.DataInfoSetVal("CapResultY4", Screw_4_dy);
            					//DataInfoManager.Instance.DataInfoSetVal("CapResultX5", CapResultY5);
            					//MessageBox.Show(string.Format("1st picture taken \n X5: {0} \n Y5: {1}", CapResultX5.ToString(), CapResultY5.ToString()));
            					break;
            			}
        			}
        			else
    	...
    	if(CurStep==83)
    	{
    		...
    		else
    		{
    			...
    			bool tempCapCalcFinish = true;
    			int alignResultArrayCount = ScrewNum;
    			if (alignResultArrayCount > AlignArrayMaxCount){ //if count is larger than 4
    				alignResultArrayCount = ScrewNum - AlignArrayMaxCount; //5 - 4 = 1
    			}
    			for(int i=0;i<alignResultArrayCount;i++)
    			{
    				if (vpAlignResultArray[i].Result == VpAlignEnum.VpAlign_Finish) //error check loop
        			{
            			switch(i)
    					{
    						case 0:
    							Screw_5_dx = vpAlignResultArray[i].Offset.X;
    							Screw_5_dy = vpAlignResultArray[i].Offset.Y;
    							//MessageBox.Show(string.Format("Step 83 \n CapResultX5: {0} \n CapResultY5 {1}", Screw_5_dx.ToString(), Screw_5_dy.ToString()));
    							break;
    					}
    				} else {
    					tempCapCalcFinish = false;
    	...
    	if(CurStep==113)
    	{
    		MessageBox.Show("align handle active for step 113");
    		if(sender==null)
    		{
    			SystemPause=true;
    			if(UpCamEndErrCount>=UpCamEndErrCountMax)
    	...
    	*/
    	...
    	//MessageBox.Show(string.Format("OnAlignRecieved event thrown step: {0}", CurStep.ToString()));
	...
	{
    	//VpCaptureResult vpCap = (VpCaptureResult)sender;
    	MessageBox.Show(string.Format("offset recieved event step: {0}", CurStep.ToString()));
	}
